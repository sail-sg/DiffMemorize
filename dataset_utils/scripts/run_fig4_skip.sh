...
arch=$4
embed=$5
skip=$6 # e.g.1,1,1,1,1,1,1,1,1
torchrun --nproc_per_node 1 \
         --nnodes $WORLD_SIZE \
         --node_rank $RANK \
         --master_addr $MASTER_ADDR \
         --master_port $MASTER_PORT \
         train.py --outdir=$savedir --data=datasets/cifar10/data_size/cifar10-$size.zip --cond=0 --arch=$arch --augment=0.0 --window-size=0.0 --precond=$precond --seed=1024 --duration=2000 --num-blocks=$num_blocks --num-channels=$num_channels --embed=$embed --fp16=False --lr=2e-4 --skip-connections=$skip

# Step III: Generate 10k samples for each model snapshot and evaluate the memorization
outdir=$savedir/00000-cifar10-$size-uncond-$arch-$precond-gpus8-batch512-fp32
torchrun --nproc_per_node 1 \
         --nnodes $WORLD_SIZE \
         --node_rank $RANK \
         --master_addr $MASTER_ADDR \
