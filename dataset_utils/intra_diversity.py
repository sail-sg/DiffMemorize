...
def open_cifar10_certain_class(tarball: str, *, max_images: Optional[int], class_num: Optional[int]):
    images = []
    labels = []
    ...
    assert balanced_images.shape == (max_idx, 32, 32, 3) and balanced_images.dtype == np.uint8
    assert balanced_labels.shape == (max_idx,) and balanced_labels.dtype in [np.int32, np.int64]
...
def interpolate_cifar10_certain_class(tarball: str, *, max_images: Optional[int], inter_rate: Optional[float]):
    ...
    assert cifar10_images.shape == (2000, 32, 32, 3) and cifar10_images.dtype == np.uint8
    assert cifar10_labels.shape == (2000,) and cifar10_labels.dtype in [np.int32, np.int64]
    assert np.min(cifar10_images) == 0 and np.max(cifar10_images) == 255

    assert imagenet_images.shape == (2000, 32, 32, 3) and imagenet_images.dtype == np.uint8
    assert imagenet_labels.shape == (2000,) and imagenet_labels.dtype in [np.int32, np.int64]
    ...
    labels = np.concatenate([cifar10_labels[:cifar10_num], imagenet_labels[:imagenet_num]], axis=0)
...
def open_dataset(source, *, max_images: Optional[int], class_num: Optional[int], inter_rate: Optional[float]):
    if os.path.isdir(source):
        ...
        if os.path.basename(source) == 'cifar-10-python.tar.gz':
            return open_cifar10_certain_class(source, max_images=max_images, class_num=class_num)
        elif os.path.basename(source) == 'cifar10_dog_2000-32x32.zip':
            return interpolate_cifar10_certain_class('dog', max_images=max_images, inter_rate=inter_rate)
        elif os.path.basename(source) == 'cifar10_cat_2000-32x32.zip':
            return interpolate_cifar10_certain_class('cat', max_images=max_images, inter_rate=inter_rate)
...
@click.option('--inter-rate', help='Interpolation rate between cifar10 and imagenet', metavar='FLOAT', type=float)
...
def main(
    ...
    num_files, input_iter = open_dataset(source, max_images=max_images, class_num=class_num, inter_rate=inter_rate)
