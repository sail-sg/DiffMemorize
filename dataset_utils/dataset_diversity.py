...
def open_cifar10_diversity(tarball: str, *, max_images: Optional[int], num_classes: Optional[int]):
    images = []
    labels = []
    ...
    for c in range(num_classes):
        ...
        if c == num_classes - 1:
            balanced_images.append(images_class[:(max_idx - num_each_class * (num_classes - 1))])
            balanced_labels.append(labels_class[:(max_idx - num_each_class * (num_classes - 1))])
        else:
...
def open_dataset(source, *, max_images: Optional[int], num_classes: Optional[int],):
    if os.path.isdir(source):
        ...
        if os.path.basename(source) == 'cifar-10-python.tar.gz':
            return open_cifar10_diversity(source, max_images=max_images, num_classes=num_classes)
...
def main(
    ...
    num_files, input_iter = open_dataset(source, max_images=max_images, num_classes=num_classes)
